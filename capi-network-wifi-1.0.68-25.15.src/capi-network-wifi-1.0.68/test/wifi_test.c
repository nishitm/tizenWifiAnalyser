...
static bool __test_found_ap_callback(wifi_ap_h ap, void *user_data)
{
	...
	//app_control_add_extra_data(app_control,"control",ap_name);
	//strcpy(ap_names[ap_count++],ap_name);
	...
	char *bssid;
	if (wifi_ap_get_bssid(ap,&bssid) == WIFI_ERROR_NONE){
			printf("BSSID : %s\n", bssid);		
	}
	//else
		//strcpy(bssid,"rohit");
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	char ap_name_bssid[200];
	strcpy(ap_name_bssid,ap_name);
	strcat(ap_name_bssid,"						BSSID-");
	strcat(ap_name_bssid,bssid);
	    
	app_control_add_extra_data(app_control,"ap_name",ap_name_bssid);
	int error = app_control_send_launch_request(app_control,NULL,NULL);

	if(error==APP_CONTROL_ERROR_NONE)
	{
		printf("Applcation Favorite Launched Succesfully\n");
	}
	else
	{
		printf("Applcation Launch failure with error code %u\n",error);
	}
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	app_control_set_app_id(app_control,"org.tizen.favorite");
	app_control_add_extra_data(app_control,"connect",ap_name);
...
int test_foreach_found_aps(void)
{
	...
	// app_control_set_app_id(app_control,"org.tizen.favorite");
	// app_control_add_extra_data(app_control,"control","2");    
	// int error = app_control_send_launch_request(app_control,NULL,NULL);
	...
	// app_control_set_app_id(app_control,"org.tizen.favorite");
	// app_control_add_extra_data(app_control,"control","3");    
	// int error1 = app_control_send_launch_request(app_control,NULL,NULL);

	// if(error1==APP_CONTROL_ERROR_NONE)
	// {
	// 	printf("Applcation Favorite Launched Succesfully");
	// }
	// else
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	app_control_set_operation(app_control,APP_CONTROL_OPERATION_DEFAULT);
	app_control_set_app_id(app_control,"org.tizen.favorite");
	app_control_add_extra_data(app_control,"connecting","data");
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	error = app_control_send_launch_request(app_control,NULL,NULL);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    serv_addr.sin_addr.s_addr = inet_addr(ip); // IP of server..
    ...
    fp = fopen(dir, "ab"); // name of the file
    if(NULL == fp)
    {
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	app_control_set_app_id(app_control,"org.tizen.favorite");
	app_control_add_extra_data(app_control,"receiving",dir);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	app_control_set_app_id(app_control,"org.tizen.favorite");
	app_control_add_extra_data(app_control,"received",dir);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    serv_addr.sin_port = htons(5000);                                    // port no
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	app_control_set_app_id(app_control,"org.tizen.favorite");
	app_control_add_extra_data(app_control,"sending",f);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    while(1)
    {
        ...
        FILE *fp = fopen(f,"rb");                              // file that want to be send
        if(fp==NULL)
        {
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		app_control_set_app_id(app_control,"org.tizen.favorite");
		app_control_add_extra_data(app_control,"sent",f);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	app_control_set_app_id(app_control,"org.tizen.favorite");
	app_control_add_extra_data(app_control,"control","1");
